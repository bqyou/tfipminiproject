...
import { Profile, Match, TextMatch } from '../models/models';
import { SQLResponse, isProfileCompleted, SwipesStatus, YesNo } from '../models/response';
...
export class OperationService {
  ...
  updateProfile(match: Match){
    ...
    formData.append('aboutMe', match.aboutMe)
    const token = localStorage.getItem('token')
    const header = new HttpHeaders().set('Authorization', 'Bearer ' + token)
